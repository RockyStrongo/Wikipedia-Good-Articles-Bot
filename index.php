...
function wikiGoodArticle()
{
	...
	if (sizeof($images) == 0) {
		...
		foreach ($imagesurlarray as $k => $v) {
			...
			//save image
			$imgtemp = '/hello' . $it . '.' . $extension;

			$dir = sys_get_temp_dir();
			$tmp = tempnam($dir, $imgtemp);

			$context = stream_context_create($options);
			file_put_contents($tmp, file_get_contents($thisurl, false, $context));
			// file_put_contents($imgtemp, file_get_contents($thisurl, false, $context));	
			$it++;
			array_push($imagestotweet, $tmp);

		}

		print_r($imagestotweet);
